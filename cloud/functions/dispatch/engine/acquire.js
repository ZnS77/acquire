...
exports.stockPrice = exports.AcquireGame = void 0;
exports.wouldMergeSafes = wouldMergeSafes;
exports.isGameOver = isGameOver;
exports.createInitialState = createInitialState;
exports.reduce = reduce;
exports.redact = redact;
const data_1 = require("./data");
Object.defineProperty(exports, "stockPrice", { enumerable: true, get: function () { return data_1.stockPrice; } });
...
function emptyShares() {
    const s = {};
    for (const id of data_1.COMPANY_IDS)
        s[id] = 0;
    return s;
}
...
function cellAt(state, tile) {
    const { col, row } = (0, data_1.parseTile)(tile);
    return state.board[row][col];
}
function setCell(state, tile, value) {
    const { col, row } = (0, data_1.parseTile)(tile);
...
function neighbors(tile) {
    ...
    const out = [];
    if (col > 0)
        out.push((0, data_1.makeTile)(col - 1, row));
    if (col < data_1.COLS - 1)
        out.push((0, data_1.makeTile)(col + 1, row));
    if (row > 0)
        out.push((0, data_1.makeTile)(col, row - 1));
    if (row < data_1.ROWS - 1)
        out.push((0, data_1.makeTile)(col, row + 1));
    return out;
}
/** Flood-fill all occupied (non-null) cells connected to `start`, inclusive. */
function connectedTiles(state, start) {
...
function refreshCompany(state, id) {
    const co = state.companies[id];
    co.size = companySize(state, id);
    co.active = co.size > 0;
    co.safe = co.size >= data_1.SAFE_SIZE;
}
...
// ---------- setup ----------
function createInitialState(players, seed, names) {
    const board = Array.from({ length: data_1.ROWS }, () => Array.from({ length: data_1.COLS }, () => null));
    const companies = {};
    for (const meta of data_1.COMPANY_META) {
        companies[meta.id] = {
            id: meta.id,
            name: meta.name,
            tier: meta.tier,
            ...
            safe: false,
            active: false,
            sharesLeft: data_1.SHARES_PER_COMPANY,
        };
    }
    const rng = (0, rng_1.makeRng)(seed);
    let bag = (0, rng_1.shuffle)((0, data_1.allTiles)(), rng);
    const hands = {};
    const pubPlayers = players.map((id, i) => ({
        id,
        name: names?.[i] ?? id,
        cash: data_1.STARTING_CASH,
        shares: emptyShares(),
        handCount: 0,
        connected: true,
    }));
    for (const p of pubPlayers) {
        const hand = bag.slice(0, data_1.HAND_SIZE);
        bag = bag.slice(data_1.HAND_SIZE);
        hands[p.id] = hand;
...
// ---------- placement ----------
function availableCompanies(state) {
    return data_1.COMPANY_IDS.filter((id) => !state.companies[id].active);
}
function activeCompanies(state) {
    return data_1.COMPANY_IDS.filter((id) => state.companies[id].active);
}
/** Draw one tile from the bag into a player's hand (if any remain). */
function drawTile(state, pid) {
...
function endTurn(state, pid) {
    if (state.phase !== 'confirm')
        return err(state, '当前不是回合确认阶段。');
    if (currentPlayer(state).id !== pid)
        return err(state, '还没轮到你。');
...
function handlePlacement(state, pid, tile) {
    ...
    if (involved.length === 1) {
        ...
        const id = involved[0];
        for (const t of connectedTiles(state, tile))
            setCell(state, t, id);
...
function chooseFoundedCompany(state, pid, company) {
    ...
    if (co.active)
        return err(state, '该公司已在场上。');
    for (const t of state.pendingFounding.componentTiles)
        setCell(state, t, company);
...
function wouldMergeSafes(state, tile) {
    ...
    while (stack.length) {
        ...
        for (const n of neighbors(t)) {
            const c = cellAt(state, n);
            if (c && c !== 'unincorp' && state.companies[c].safe)
                safes.add(c);
            if (!seen.has(n) && cellAt(state, n) !== null) {
                seen.add(n);
                stack.push(n);
...
function resolveMerger(state, pid, survivor, order) {
    const pm = state.pendingMerger;
    if (!pm || !pm.awaitingResolve)
        return err(state, '当前无需选择保留方。');
    if (pm.triggerer !== pid)
        return err(state, '只有触发者可选择保留方。');
    if (!pm.candidates.includes(survivor))
        return err(state, '保留方必须是可选公司之一。');
    const defunct = pm.involved.filter((id) => id !== survivor);
    // `order` (optional) lets the triggerer set processing order of the defunct.
    const ordered = order.length === defunct.length && order.every((id) => defunct.includes(id))
        ? order
...
function finalizeMergerSurvivor(state, survivor, involved) {
    const pm = state.pendingMerger;
...
function beginNextDefunct(state) {
    const pm = state.pendingMerger;
...
function payMergerBonuses(state, defunct) {
    const co = state.companies[defunct];
    const price = (0, data_1.stockPrice)(co.tier, co.size);
...
function pay(state, pid, amount) {
    playerById(state, pid).cash += amount;
    log(state, `${nameOf(state, pid)} 获得分红 ${amount} 元。`);
}
function mergerDispose(state, pid, keep, sell, trade) {
    const pm = state.pendingMerger;
    ...
    if (keep < 0 || sell < 0 || trade < 0)
        return err(state, '处置数量不能为负。');
    if (keep + sell + trade !== held)
        return err(state, `处置数量必须合计为 ${held} 股。`);
    if (trade % 2 !== 0)
        return err(state, '换股必须以 2 股为单位（2:1）。');
    ...
    log(state, `${nameOf(state, pid)} 处置 ${co.name}：留 ${keep} / 卖 ${sell} / 换 ${survivorOut}。`);
    pm.disposalIndex += 1;
...
function completeMerger(state) {
    const pm = state.pendingMerger;
    const survivor = pm.survivor;
    // Survivor takes over the entire connected blob.
    for (const t of pm.componentTiles)
        setCell(state, t, survivor);
...
// ---------- buying ----------
function buyShares(state, pid, buy) {
    const player = playerById(state, pid);
    let total = 0;
    let cost = 0;
    for (const id of data_1.COMPANY_IDS) {
        const want = buy[id] ?? 0;
        if (want < 0)
            return err(state, '购买数量不能为负。');
        if (want === 0)
            continue;
        const co = state.companies[id];
        if (!co.active)
            return err(state, `${co.name} 未开业，不能购买。`);
        if (want > co.sharesLeft)
            return err(state, `${co.name} 发行余量不足。`);
        total += want;
        cost += want * (0, data_1.stockPrice)(co.tier, co.size);
    }
    if (total > data_1.MAX_BUY_PER_TURN)
        return err(state, `单回合最多购买 ${data_1.MAX_BUY_PER_TURN} 股。`);
    if (cost > player.cash)
        return err(state, '现金不足。');
    for (const id of data_1.COMPANY_IDS) {
        const want = buy[id] ?? 0;
        if (want > 0) {
    ...
    player.cash -= cost;
    if (total > 0)
        log(state, `${nameOf(state, pid)} 购买了 ${total} 股，花费 ${cost} 元。`);
    else
        log(state, `${nameOf(state, pid)} 跳过购买。`);
    drawAndPause(state);
    return { state };
}
// ---------- endgame ----------
function isGameOver(state) {
    ...
    if (active.some((id) => state.companies[id].size >= data_1.END_SIZE))
        return true;
    if (active.length > 0 && active.every((id) => state.companies[id].safe))
        return true;
    if (state.bag.length === 0) {
...
function declareEnd(state, pid) {
    if (currentPlayer(state).id !== pid)
        return err(state, '还没轮到你。');
    const active = activeCompanies(state);
    const canEnd = (active.length > 0 && active.every((id) => state.companies[id].safe)) ||
        active.some((id) => state.companies[id].size >= data_1.END_SIZE);
    if (!canEnd)
        return err(state, '当前不满足终局条件，不能宣布结束。');
    finalizeGame(state);
    return { state };
}
...
function redact(state, viewerId) {
    return {
        phase: state.phase,
        board: state.board,
        ...
        yourLastDraw: state.pendingDraw && state.pendingDraw.player === viewerId
...
// ---------- reducer ----------
function reduce(state, playerId, action) {
    const next = clone(state);
    if (action.type === 'START') {
        if (next.phase !== 'lobby')
            return err(state, '游戏已经开始。');
        if (next.players.length < 2)
            return err(state, '至少需要 2 名玩家。');
        next.phase = 'placing';
        log(next, `游戏开始，由 ${nameOf(next, currentPlayer(next).id)} 先手。`);
        return { state: next };
    }
    if (next.phase === 'ended')
        return err(state, '游戏已结束。');
    switch (action.type) {
        case 'PLACE_TILE': {
            if (next.phase !== 'placing')
                return err(state, '现在不是落子阶段。');
            if (currentPlayer(next).id !== playerId)
                return err(state, '还没轮到你。');
            const hand = next.hands[playerId] ?? [];
            if (!hand.includes(action.tile))
                return err(state, '你手上没有这块板。');
            if (cellAt(next, action.tile) !== null)
                return err(state, '该位置已被占用。');
        ...
        case 'BUY_SHARES': {
            if (next.phase !== 'buying')
                return err(state, '现在不是购买阶段。');
            if (currentPlayer(next).id !== playerId)
                return err(state, '还没轮到你。');
